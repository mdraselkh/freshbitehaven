        ...
        <div className="hidden lg:flex flex-col items-start justify-start gap-3 p-4 border-2 shadow-lg w-1/4 h-[550px] overflow-y-auto">
          ...
          {categories.map((category, index) => (
            ...
            >
              ...
              {subcategories
                .filter((subcategory) => subcategory.categoryId === category.id) // Filter subcategories by category
                .map((subcategory) => (
                  <button
                    key={subcategory.id} // Assuming each subcategory has a unique id
                    href="#"
                    onClick={() =>
                      handleItemClick(
                    ...
                    className="text-gray-700 hover:text-[#ffbd59] ml-4" // Optional styling
                  >
                    {"— " + subcategory.name}(
                    {products.filter(
                      (product) =>
